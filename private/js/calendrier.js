...
function resetFocusDate () {
    focusDate = new Date(Date.now() - 24 * 3600000 * (today.getDay() - (today.getDay() === 0 ? -6 : 1)));
}
...
function updateCalendar (focusDateLun, idE) {
    ...
    document.getElementById('titre_calendrier').textContent = yearFocus + ' Semaine ' + focusDateLun.getWeekNumber();
    document.getElementById('calendrier').setAttribute('id-element', '' + idE);
...
document.getElementById('btn_min').addEventListener('click', () => {
    ...
    updateCalendar(focusDate, parseInt(document.getElementById('calendrier').getAttribute('id-element')));
});
