    ...
    .then(function (crenData) {
        ...
        .then(function (elemData) {
            fetch('api/utilisateur/bynum', {
                credentials: 'same-origin',
                method: 'POST',
                body: reservData.id_Utilisateur,
                headers: new Headers({'Content-type': 'application/json'}),
            })
            .then(function (userData) {
                ...
                if (reservData.validation === 0) {
                    ...
                    inputVal.addEventListener('click', () => validateReserv(reservData.id));
                    form1.appendChild(inputVal);
                    divButtons.appendChild(form1);
                    ...
                    form2.appendChild(inputRef);
                    divButtons.appendChild(form2);
...
function validateReserv(id, valid = true) {
    fetch('api/reservation/validate', {
        credentials: 'same-origin',
        method: 'POST',
        body: {id_Reservation: id, validate: (!valid ? false : true)},
        headers: new Headers({'Content-type': 'application/json'}),
    })
    .catch(err => console.error(err));
}
